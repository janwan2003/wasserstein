...
from nmr_spectrum import Spectrum
...
def dualdeconv2(exp_sp, thr_sps, penalty, quiet=True, solver=LpSolverDefault):
    ...
    # Normalization check:
    assert np.isclose(sum(x[1] for x in exp_confs), 1), (
        "Mixture's spectrum not normalized."
    )
    for i, thrcnf in enumerate(thr_confs):
        assert np.isclose(sum(x[1] for x in thrcnf), 1), (
            "Component's spectrum %i is not normalized." % i
        )
    ...
    interval_lengths = [
        common_horizontal_axis[i + 1] - common_horizontal_axis[i] for i in range(n - 1)
    ]
    if not quiet:
    ...
    # objective function:
    exp_vec = intensity_generator(
        exp_confs, common_horizontal_axis
    )  # generator of mixture's intensity observations
    ...
    for j in range(k):
        ...
        program += (
            lp.lpSum(v * x for v, x in zip(thr_vec, lpVars) if v > 0.0) <= 0,
            "P%i" % (j + 1),
        )
    if not quiet:
        print("tsk tsk")
    ...
    for i in range(n - 1):
        program += (
            lpVars[i] - lpVars[i + 1] <= interval_lengths[i],
            "EpsPlus_%i" % (i + 1),
        )
        program += (
            lpVars[i] - lpVars[i + 1] >= -interval_lengths[i],
    ...
    if not np.isclose(sum(probs) + sum(abyss), 1.0, atol=len(abyss) * 1e-03):
        warn(
            """In dualdeconv2:
    ...
    return {
        "probs": probs,
        "trash": abyss,
        "fun": lp.value(program.objective),
        "status": program.status,
        "common_horizontal_axis": common_horizontal_axis,
    }


def dualdeconv2_alternative(
    exp_sp, thr_sps, penalty, quiet=True, solver=LpSolverDefault
):
    """
    ...
    for i, thrcnf in enumerate(thr_confs):
        assert np.isclose(sum(x[1] for x in thrcnf), 1), (
            "Component's spectrum %i not normalized." % i
        )
...
def dualdeconv3(
    exp_sp, thr_sps, penalty, penalty_th, quiet=True, solver=LpSolverDefault
):
    """
    ...
    program += (
        lp.lpSum(v * x for v, x in zip(exp_vec, lpVars[: n - 1] + [0])).addInPlace(
            lp.lpSum(v * x for v, x in zip([-1, 0, 0, -1], lpVars[n - 1 :]))
        ),
        "Dual_objective",
    )
    ...
    for j in range(k):
        ...
        program += (
            lp.lpSum(
                v * x for v, x in zip(thr_vec, lpVars[: n - 1] + [0]) if v > 0.0
            ).addInPlace(
                lp.lpSum(v * x for v, x in zip([-1, 0, 1, -1], lpVars[n - 1 :]))
            )
            <= 0,
            "P_%i" % (j + 1),
        )
    ...
    program += (
        lp.lpSum(v * x for v, x in zip(exp_vec, lpVars[: n - 1] + [0])).addInPlace(
            lp.lpSum(v * x for v, x in zip([0, 1, -1, 0], lpVars[n - 1 :]))
        )
        <= 0,
        "p0_prime",
    )
    ...
    try:
        for i in range(n - 2):
            ...
            program += (
                lpVars[i + 1] - lpVars[i] <= interval_lengths[i],
                "epsilon_minus_%i" % (i + 1),
            )
    except IndexError:  # linear program makes no sense if n<=2 (n-number of points in mixture's spectrum)
        pass
    ...
    return {
        "probs": probs,
        "noise_in_components": p0_prime,
        "trash": abyss,
        "components_trash": abyss_th,
        "fun": lp.value(program.objective),
        "status": program.status,
        "common_horizontal_axis": common_horizontal_axis,
    }
...
def dualdeconv4(
    ...
    ALGORITHM OVERVIEW:
    ------------------
    This algorithm solves the problem of spectrum deconvolution using the Wasserstein metric
    and linear programming. It identifies the proportions of reference spectra in a given
    mixture spectrum, while accounting for noise in both the mixture and reference spectra.

    TRASH POINTS (ŚMIETNIKI) EXPLANATION:
    ------------------------------------
    The algorithm uses "trash points" or "abyss" to handle noise:

    1. Mixture's spectrum trash point: Collects intensity that appears in the mixture
       but cannot be explained by any component spectrum. This represents noise or
       contaminants in the mixture. The cost of transporting signal to this trash
       is controlled by the 'penalty' parameter.

    2. Components' spectra trash point: Collects intensity from reference spectra
       that doesn't match anything in the mixture. This represents artifacts or noise
       in the reference spectra. The cost of transportation to this trash is controlled
       by the 'penalty_th' parameter.

    The mathematical formulation ensures that direct transportation between the two
    trash points is suboptimal, forcing the algorithm to assign signal properly.

    TECHNICAL FORMULATION:
    --------------------
    This is the dual formulation of the Wasserstein L1 metric minimization problem.
    The variables (lpVars) represent potentials in the dual formulation of the
    optimal transport problem, with the last four variables handling the trash points.

    The constraints enforce:
    - The 1-Lipschitz condition (epsilon constraints)
    - The dual constraints for each component spectrum (P_i constraints)
    - The trash point conditions (g_i and g_prime_i constraints)
    ...
    # The 'probs' values represent the proportions of each component spectrum in the mixture
    # These are retrieved from the shadow prices (dual variables) of component constraints
    probs = [round(constraints["P_%i" % i].pi, 12) for i in range(1, k + 1)]

    # 'p0_prime' represents the proportion of noise in the combination of components spectra
    # This is extracted from the shadow price of the p0_prime constraint
    p0_prime = round(constraints["p0_prime"].pi, 12)

    # Convert generator to list for potential reuse
    exp_vec = list(intensity_generator(exp_confs, common_horizontal_axis))

    # 'abyss' (trash) represents the intensity at each point that couldn't be explained
    # by any component spectrum - this is effective noise in the mixture's spectrum
    abyss = [round(constraints["g_%i" % i].pi, 12) for i in range(1, n + 1)]

    # 'abyss_th' (components_trash) represents the intensity at each point in the component
    # spectra that doesn't match to the mixture - this is noise in the reference spectra
    abyss_th = [round(constraints["g_prime_%i" % i].pi, 12) for i in range(1, n + 1)]

    # Sanity check - the sum of all signal proportions (components + trash) should equal 1
    # since all spectra are normalized
    if not np.isclose(sum(probs) + sum(abyss), 1.0, atol=len(abyss) * 1e-03):
        warn(
            """In dualdeconv4:
                Proportions of signal and noise sum to %f instead of 1.
    ...
    # Return results dictionary with all components of the solution
    # fun: objective function value represents the Wasserstein distance plus penalty
    return {
        "probs": probs,
        "noise_in_components": p0_prime,
        "trash": abyss,
        "components_trash": abyss_th,
        "fun": lp.value(program.objective) + penalty,
        "status": program.status,
        "common_horizontal_axis": common_horizontal_axis,
    }
...
def estimate_proportions(
    ...
    try:
        ...
        print(
            "Could not retrieve the confs list. Is the supplied spectrum an object of class Spectrum?"
        )
        raise

    assert what_to_compare == "concentration" or what_to_compare == "area", (
        "Comparison of %s is not supported" % what_to_compare
    )

    if what_to_compare == "concentration":
        for i, sp in enumerate(query):
            assert sp.protons is not None, (
                "Component "
                + str(i)
                + " doesn't have the number of protons defined. Please define sp.protons attribute."
            )

    is_NMR_spectrum = [isinstance(sp, NMRSpectrum) for sp in [spectrum] + query]
    assert all(is_NMR_spectrum) or not any(is_NMR_spectrum), (
        "Spectra provided are of mixed types. \
            Please assert that either all or none of the spectra are NMR spectra."
    )
    nmr = all(is_NMR_spectrum)

    if not nmr:
        assert all(x[0] >= 0.0 for x in exp_confs), "Found peaks with negative masses!"
    ...
    if any(x[1] < 0 for x in exp_confs):
        ...
        exp_confs = [(ppm, intsy if intsy >= 0 else 0.0) for ppm, intsy in exp_confs]

    if not abs(sum(x[1] for x in exp_confs) - 1.0) < 1e-08:
    ...
    assert abs(sum(x[1] for x in exp_confs) - 1.0) < 1e-08, (
    ...
    for i, q in enumerate(query):
        ...
        if any(x[1] < 0 for x in q_confs):
            ...
            print("Setting negative intensities to zero.")
            q_confs = [(ppm, intsy if intsy >= 0 else 0.0) for ppm, intsy in q_confs]

        if not abs(sum(x[1] for x in q_confs) - 1.0) < 1e-08:
        ...
        if not nmr:
            assert all(x[0] >= 0 for x in q.confs), (
                "Component's spectrum %i has negative masses!" % i
            )
            preprocessed_query.append(Spectrum(confs=q_confs))
        else:
            preprocessed_query.append(
                NMRSpectrum(confs=q_confs, protons=query[i].protons)
            )
    ...
    for i in progr_bar(range(k), desc="Initial filtering of formulas"):
        ...
        matching_current = (
            MDC == 0.0
            or sum(
                x[1] for x in misc.extract_range(exp_confs, mn - MTD_max, mx + MTD_max)
            )
            >= MDC
        )
    ...
    for conf_id, cur_conf in progr_bar(
        enumerate(exp_confs), desc="Splitting the mixture's spectrum into chunks"
    ):
    ...
    exp_conf_chunks.append(matching_confs)
    chunk_TICs = [
        sum(exp_confs[i][1] for i in chunk_list) for chunk_list in exp_conf_chunks
    ]
    if verbose:
    ...
    for current_chunk_ID, conf_IDs in progr_bar(
        enumerate(exp_conf_chunks),
        desc="Deconvolving chunks",
        ...
        if chunk_TICs[current_chunk_ID] < 1e-16:
            ...
            if verbose:
                print(
                    "Chunk %i is almost empty - skipping deconvolution"
                    % current_chunk_ID
                )
            for i in conf_IDs:
            ...
            chunkSp.normalize()
            components_spectra_IDs = [
                i for i, c in enumerate(chunkIDs) if c == current_chunk_ID
            ]
            ...
            while not success:
                ...
                if rerun > max_reruns:
                    raise RuntimeError(
                        "Failed to deconvolve a fragment of the mixture's spectrum:\
                                                 (%f, %f)"
                        % chunk_bounds[current_chunk_ID]
                    )
                if MTD_th is None:
                    dec = dualdeconv2(chunkSp, thrSp, MTD, quiet=True, solver=solver)
                else:
                    dec = dualdeconv4(
                        chunkSp, thrSp, MTD, MTD_th, quiet=True, solver=solver
                    )
                if dec["status"] == 1:
                    success = True
                else:
                    warn(
                        "Rerunning computations for chunk %i due to status %s"
                        % (current_chunk_ID, lp.LpStatus[dec["status"]])
                    )
            if verbose:
                ...
                if MTD_th is not None:
                    print(
                        "Noise proportion in combination of components' spectra:",
                        dec["noise_in_components"],
                    )
            ...
            rescaled_vortex = [
                element * chunk_TICs[current_chunk_ID] for element in dec["trash"]
            ]
            vortex = vortex + rescaled_vortex
            common_horizontal_axis = (
                common_horizontal_axis + dec["common_horizontal_axis"]
            )

            if MTD_th is not None:
                p0_prime = (
                    p0_prime + dec["noise_in_components"] * chunk_TICs[current_chunk_ID]
                )
                rescaled_vortex_th = [
                    element * chunk_TICs[current_chunk_ID]
                    for element in dec["components_trash"]
                ]
    ...
    exp_confs_from_outside_cha = (
        exp_confs_outside_chunks + exp_confs_in_almost_empty_chunks
    )
    ...
    if MTD_th is not None:
        ...
        vortex_th = list(zip(common_horizontal_axis, vortex_th)) + [
            (el[0], 0.0) for el in exp_confs_from_outside_cha
        ]
    ...
    if not np.isclose(sum(proportions) + sum(vortex), 1.0, atol=len(vortex) * 1e-03):
        warn(
            """In estimate_proportions:
Proportions of signal and noise sum to %f instead of 1.
This may indicate improper results.
Please check the deconvolution results and consider reporting this warning to the authors.
                        """
    ...
    if compare_area:
        if MTD_th is not None:
            return {
                ...
                "noise": vortex,
                "noise_in_components": vortex_th,
                "proportion_of_noise_in_components": p0_prime,
                "common_horizontal_axis": common_horizontal_axis,
        ...
        rescaled_proportions = [
            prop / prot for prop, prot in zip(proportions, queries_protons)
        ]
        rescaled_proportions = [
            prop / sum(rescaled_proportions) for prop in rescaled_proportions
        ]
        return {
