...
G01 = dia_matrix(
    (_G01, sparse1.offsets), shape=(sparse1.n, sparse1.m), dtype=np.float64
)
G02 = dia_matrix(
    (_G02, sparse2.offsets), shape=(sparse2.n, sparse2.m), dtype=np.float64
)
...
def plot_heatmap_grid(metric_name, data_s1, data_s2, xvals, yvals):
    ...
    filepath = (
        f"plots/{save_path}/heatmaps_close/{metric_name.lower().replace(' ', '_')}.png"
    )
...
for metric in metrics_s1.keys():
    plot_heatmap_grid(
        metric, metrics_s1[metric], metrics_s2[metric], regm2_values, regm1_values
    )
