...
class Spectrum:
    def __init__(
        ...
        **other,
        ...
        if formula != "" and confs is not None:
            raise ValueError("Formula and confs cannot be set at the same time!")
        elif confs is not None:
            self.set_confs(confs)
        elif formula != "":
            self.set_confs(
                self.confs_from_formula(formula, threshold, total_prob, charge, adduct)
            )
        else:
            self.empty = True
            self.confs = []
    ...
    @staticmethod
    def confs_from_formula(
        formula, threshold=0.001, total_prob=None, charge=1, adduct=None
    ):
        ...
        if total_prob is not None:
            isospec = IsoSpecPy.IsoTotalProb(
                formula=formula,
            ...
            isospec = IsoSpecPy.IsoThreshold(
                formula=formula, threshold=threshold, absolute=False, get_confs=False
            )
        confs = [(x[0] / abs(charge), x[1]) for x in zip(isospec.masses, isospec.probs)]
        return confs
    ...
    def ScalarProduct(spectra, weights):
        ...
        while Q != []:
            ...
            if conf_idx < len(spectra[spectre_no]):
                heapq.heappush(
                    Q, (spectra[spectre_no].confs[conf_idx], spectre_no, conf_idx)
                )
        ret.merge_confs()
        return ret
    ...
    def WSDistance(self, other):
        if not np.isclose(sum(x[1] for x in self.confs), 1.0):
    ...
    def WSGradient(self, target, epsilon=1e-5):
        ...
        mzs = np.array([mz for mz, inten in self.confs])
        intensities = np.array([inten for mz, inten in self.confs])
        ...
        for i in trange(len(mzs), desc="Computing WSDistance gradient"):
            ...
            dist_plus = self.WSDistance(source_plus)
            dist_minus = self.WSDistance(source_minus)
    ...
    def add_chemical_noise(self, nb_of_noise_peaks, noise_fraction, span=1.2):
        ...
        if isinstance(span, (float, int)):
            span_increase = span
            prev_span = (min(x[0] for x in self.confs), max(x[0] for x in self.confs))
            span_move = 0.5 * (span_increase - 1) * (prev_span[1] - prev_span[0])
            span = (max(prev_span[0] - span_move, 0), prev_span[1] + span_move)
        noisex = uniform.rvs(
            loc=span[0], scale=span[1] - span[0], size=nb_of_noise_peaks
        )
    ...
    def sample_multinomial(reference, N, gain, sd):
        ...
        assert np.isclose(sum(p), 1), (
            "Spectrum needs to be normalized prior to sampling"
        )
        U = rd.multinomial(N, p)
        U = rd.normal(U * gain, np.sqrt(U * sd**2))
        retSp = Spectrum("", empty=True, label="Sampled " + reference.label)
        retSp.set_confs([(x[0], max(u, 0.0)) for x, u in zip(reference.confs, U)])
        return retSp
    ...
    def trim_negative_intensities(self):
        ...
        """
        self.confs = [(mz, intsy if intsy >= 0 else 0.0) for mz, intsy in self.confs]
    ...
    def centroid(self, max_width, peak_height_fraction=0.5):
        ...
        for p in peak_indices:
            ...
            while (
                p + right_shift < n - 1
                and mz[p + right_shift] - mz[p] < max_dist
                and intsy[p + right_shift] <= current_intsy
                and intsy[p + right_shift] > target_intsy
            ):
                right_shift += 1
            ...
            while (
                p - left_shift > 1
                and mz[p] - mz[p - left_shift] < max_dist
                and intsy[p - left_shift] <= current_intsy
                and intsy[p - left_shift] > target_intsy
            ):
                left_shift += 1
            ...
            if not lx1 <= lx <= lx2:
                raise RuntimeError(
                    "Failed to interpolate the left boundary mz value of the peak at %f"
                    % current_mz
                )
            rx = (target_intsy - ry1) * (rx2 - rx1) / (ry2 - ry1) + rx1
            if not rx1 <= rx <= rx2:
                raise RuntimeError(
                    "Failed to interpolate the right boundary mz value of the peak at %f"
                    % current_mz
                )
            ...
            y = np.hstack(
                (
                    target_intsy,
                    intsy[(p - left_shift + 1) : (p + right_shift)],
                    target_intsy,
                )
            )
            # Integrate the area:
            ...
            if (
                cmz not in centroid_mz
            ):  # intensity errors may introduce artificial peaks
        ...
        return (
            list(zip(centroid_mz, centroid_intensity)),
            list(zip(peak_mz, peak_intensity)),
        )
    ...
    def resample(self, target_mz, mz_distance_threshold=0.05):
        ...
        y = [0.0] * lenx  # resampled intensities
        y0, y1 = (
            intsy[0],
            intsy[0],
        )  # intensities of target spectrum around the point target_mz[qi]
    ...
    def fuzzify_peaks(self, sd, step):
        ...
        """
        new_mass = np.arange(
            self.confs[0][0] - 4 * sd, self.confs[-1][0] + 4 * sd, step
        )
        ...
        A /= -2 * sd**2
        A = np.exp(A)
        new_intensity = A @ np.array(
            [i for m, i in self.confs]
    ...
    def gaussian_smoothing(self, sd=0.01, new_mz=0.01):
        ...
        if isinstance(new_mz, float):
            new_mz = np.arange(
                self.confs[0][0] - 4 * sd, self.confs[-1][0] + 4 * sd, new_mz
            )
        ...
        for mz, intsy in self.confs:
            ...
            smooth_intensity[lpid:rpid] += intsy * np.exp(-((mz - peak_mz) ** 2)) ** (
                1 / (2 * sd**2)
            )
    ...
    def filter_peaks(self, list_of_others, margin):
        ...
        """
        bounds = [
            (s.confs[0][0] - margin, s.confs[-1][0] + margin) for s in list_of_others
        ]
    ...
    def filter_against_other(self, others, margin=0.15):
        ...
        for mz, abund in self.confs:
            while index + 1 < len(other_masses) and other_masses[index + 1] < mz:
                index += 1
            if abs(mz - other_masses[index]) <= margin or (
                index + 1 < len(other_masses)
                and abs(mz - other_masses[index + 1]) <= margin
            ):
    ...
    def plot(self, show=True, profile=False, linewidth=1, **plot_kwargs):
        ...
        if profile:
            plt.plot(
                [x[0] for x in self.confs],
                [x[1] for x in self.confs],
                linestyle="-",
                linewidth=linewidth,
                label=self.label,
                **plot_kwargs,
    ...
    def plot_all(spectra, show=True, profile=False, cmap=None, **plot_kwargs):
        ...
        if not cmap:
            ...
            try:
                colors = [[0, 0, 0, 0.8]] + [
                    cmap(x, alpha=1) for x in range(len(spectra))
                ]
            except:
                colors = cmap
        i = 0
        for spectre in spectra:
            spectre.plot(show=False, profile=profile, color=colors[i], **plot_kwargs)
            i += 1
...
if __name__ == "__main__":
    ...
    plt.subplot(223)
    S.plot(show=False, profile=True)
    plt.plot([mz for mz, intsy in S.confs], [intsy for mz, intsy in S.confs], "r.")
    plt.subplot(224)
    R.plot(show=False, profile=True)
    plt.plot([mz for mz, intsy in R.confs], [intsy for mz, intsy in R.confs], "r.")
    plt.tight_layout()
    plt.show()
